...
class _SearchScreenState extends State<SearchScreen> {
  ...
  List<Map<String, String>> tickets = [
    {'flight': 'Flight A', 'departure': 'London', 'arrival': 'New York', 'price': '\$200'},
    {'flight': 'Flight B', 'departure': 'Paris', 'arrival': 'Tokyo', 'price': '\$250'},
    {'flight': 'Flight C', 'departure': 'Berlin', 'arrival': 'Dubai', 'price': '\$300'},
  ];
  ...
  void searchTickets() async {
    ...
    if (departure.isNotEmpty && arrival.isNotEmpty && date.isNotEmpty) {
      try {
        final searchData = {
          'departure': departure,
          'arrival': arrival,
        ...
        await FirebaseFirestore.instance.collection('searches').add(searchData);

        setState(() {
          tickets = [
            {'flight': 'Flight A', 'departure': departure, 'arrival': arrival, 'price': '\$200'},
            {'flight': 'Flight B', 'departure': departure, 'arrival': arrival, 'price': '\$250'},
            {'flight': 'Flight C', 'departure': departure, 'arrival': arrival, 'price': '\$300'},
          ];
        });
      } catch (e) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Please fill in all search fields.')),
      );
    }
  }
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          const AppIconText(icon: Icons.calendar_today, text: "Date"),
          TextField(
          ...
          Row(
            ...
            children: [
              const AppDoubleTextWidget(bigText: "Available Tickets", smallText: ""),
              TextButton(
                onPressed: () {
                  Navigator.push(
                    context,
                    MaterialPageRoute(
                      builder: (context) => AllTicketsScreen(tickets: tickets),
                    ),
                  );
                },
                  ...
                  child: Row(
                    children: [
                      ...
                      Column(
                        ...
                        children: [
                          Text(tickets[index]['flight']!, style: Styles.headLineStyle2),
                          Text('${tickets[index]['departure']} to ${tickets[index]['arrival']}', style: Styles.headLineStyle3),
                          Text(tickets[index]['price']!, style: Styles.textStyle),
                        ],
                      ),
                    ],
                  ),
